...
package edu.utexas.cs.tamerProject.environments.cartarm.visualizer;
...
import edu.utexas.cs.tamerProject.environments.cartarm.CartArmState;
...
public class CartArmComponent implements SelfUpdatingVizComponent, Observer {

    private final CartArmVisualizer theVisualizer;
    ...
    public CartArmComponent(CartArmVisualizer theVisualizer) {
...
//			agentPos = CartArmState.goalLoc.clone(); // show agent in goal
//		}
...
//		System.out.println("CartArmState.worldDims: " + Arrays.toString(CartArmState.worldDims));
		...
		Ellipse2D graspCircle = new Ellipse2D.Double(lastJointLoc.getX() - CartArmState.graspRadius,
													lastJointLoc.getY() - CartArmState.graspRadius,
													CartArmState.graspRadius * 2, CartArmState.graspRadius * 2);
		g.fill(graspCircle);
		...
		Ellipse2D targetDot = new Ellipse2D.Double(CartArmState.targetLoc.getX() - targetDotRadius,
												CartArmState.targetLoc.getY() - targetDotRadius,
												targetDotRadius * 2, targetDotRadius * 2);
		g.fill(targetDot);
		
		Ellipse2D targetCircle = new Ellipse2D.Double(CartArmState.targetLoc.getX() - targetCircleRadius,
													CartArmState.targetLoc.getY() - targetCircleRadius,
