...
import edu.utexas.cs.tamerProject.environments.cartarm.messages.StateResponse;

import org.rlcommunity.rlglue.codec.types.Action;
import org.rlcommunity.rlglue.codec.types.Observation;
import org.rlcommunity.rlglue.codec.types.Reward_observation_terminal;

import edu.utexas.cs.tamerProject.environments.cartarm.visualizer.CartArmVisualizer;
...
public class CartArm extends SampleableEnvBase implements
    ...
    protected final CartArmState theState;
    ...
    public TaskSpec makeTaskSpec() {
        ...
        int numArmActions = CartArmState.worldIntDims.length * 2;
        numActions = numArmActions + 5;
        theTaskSpecObject.addDiscreteAction(new IntRange(0, numActions - 1));
        theTaskSpecObject.setRewardRange(new DoubleRange(-80, 0));
        theTaskSpecObject.setExtra("EnvName:CartArm Revision:" + this.getClass().getPackage().getImplementationVersion());
    ...
    public Reward_observation_terminal env_step(Action theAction) {
        ...
        //System.out.println("Action in CartArm.env_step(): " + a);
        theState.update(a);
...
//        }
     ...
     * @param p
     */
    public CartArm(ParameterHolder p) {
    	this(p, false);
    }
    
    public CartArm(ParameterHolder p, boolean allowSecretPaths) {
        super();
        ...
        theState = new CartArmState(randomStartStates, transitionNoise, randomSeed, allowSecretPaths);
	...
	private void processArgs(String[] args) {
		System.out.println("\n[------CartArm command line args------] " + Arrays.toString(args));
...
//			}
    ...
    public String env_message(String theMessage) {
        ...
        if (theMessageObject.getTheMessageType() == rlVizLib.messaging.environment.EnvMessageType.kEnvCustom.id()) {
            ...
            if (theCustomType.equals("GETPWSTATE")) {
                ...
                int[] agentPosition=CartArmState.getDefaultInitPosition();
                if(theState!=null){
        ...
        System.err.println("We need some code written in Env Message for CartArm.. unknown request received: " + theMessage);
        Thread.dumpStack();
        return null;
    }
    ...
    public String getVisualizerClassName() {
        return CartArmVisualizer.class.getName();
    }
	...
	public Reward_observation_terminal sampleStartObs(boolean envAsking) {
		...
		rot.o = CartArmState.sampleEnvStart(envAsking);
		rot.r = 0;
		rot.terminal = 0;
		return rot;
	}
	...
	public Reward_observation_terminal sampleNextObs(Observation obs,
			Action act) {
		Reward_observation_terminal rot=new Reward_observation_terminal();
		rot.o = CartArmState.sampleNextObs(obs, act);
		rot.terminal = 0;
         ...
         */
        if (CartArmState.inGoalRegion(rot.o.intArray))
        	rot.terminal = 1;
	...
	public boolean isObsLegal(Observation obs) {
		return CartArmState.isStateLegal(obs.intArray);
	}

	@Override
	public boolean isObsTerminal(Observation obs) {
		return CartArmState.inGoalRegion(obs.intArray);
	}
}
